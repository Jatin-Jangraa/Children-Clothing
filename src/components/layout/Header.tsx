          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <Link href="/" className="flex items-center gap-2">
              <motion.div
                ...
                className="w-10 h-10 lg:w-12 lg:h-12 rounded-2xl bg-gradient-to-br from-pink-500 to-purple-500 flex items-center justify-center shadow-lg shadow-pink-200"
              >
                <span className="text-white font-bold text-lg lg:text-xl">LC</span>
              </motion.div>
              <div className="hidden sm:block">
                <h1 className="text-lg lg:text-xl font-bold bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">
                  Little Closet
                </h1>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              >
                ...
                {itemCount > 0 && (
                  <span
                    className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-gradient-to-r from-pink-500 to-purple-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center"
                  >
                    {itemCount}
                  </span>
                )}
              ...
              {session ? (
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-pink-500 to-purple-500 text-white text-sm font-medium rounded-xl shadow-lg shadow-pink-200/50 hover:shadow-xl transition-all"
                >
              ...
              <div className="p-6">
                ...
                {session ? (
                  <div className="flex items-center gap-3 p-4 mb-6 bg-gradient-to-r from-pink-50 to-purple-50 rounded-2xl">
                    ...
                    <button onClick={() => signOut()} className="p-2 text-red-500 hover:bg-red-50 rounded-xl">
                      <LogOut className="h-4 w-4" />
                    </button>
                  </div>
                ) : (
                  <button
                    onClick={() => signIn("google")}
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 mb-6 bg-gradient-to-r from-pink-500 to-purple-500 text-white font-medium rounded-xl shadow-lg"
                  >
                    <LogIn className="h-4 w-4" />
                    Sign in with Google
                  </button>
                  ...
                  {categories.map((cat) => (
                    <div key={cat.name}>
                      <Link
                        ...
                        className="block px-4 py-3 text-sm font-medium text-gray-700 hover:text-pink-500 hover:bg-pink-50 rounded-xl transition-all"
                      >
                        {cat.name}
                      </Link>
                      {cat.subcategories && (
                        <div className="pl-8 space-y-1 mt-1">
                          {cat.subcategories.map((sub) => (
                            <Link
                              ...
                              href={`/shop?category=${sub.toLowerCase().replace(/\s+/g, "-")}`}
                              onClick={() => setIsMobileMenuOpen(false)}
                              className="block px-4 py-2 text-xs text-gray-500 hover:text-pink-500 transition-colors"
                            >
                              {sub}
                            </Link>
                          ))}
                ...
                <div className="mt-8 pt-6 border-t space-y-3">
                  <a href="tel:+919876543210" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-600">
                    <Phone className="h-4 w-4" /> +91 98765 43210
                  </a>
                  <a href="mailto:hello@littlecloset.in" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-600">
